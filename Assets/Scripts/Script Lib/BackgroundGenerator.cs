...
public class BackgroundGenerator : MonoBehaviour {
	...
	void generateLevel(int size){
		Transform piece = (Transform) Instantiate(startPrefabList[0], new Vector3 (28.85f, 0, 0), Quaternion.identity);
	...
	Transform generateNextPiece(Transform prev){
		...
		do{
			...
			Debug.Log ("BG: Chosen bg piece: "+ pos + "/" + (prefabList.Count - 1));
			...
			if (cpNewPiece == null) {
				Debug.Log ("BG: Connection inpossible! Trying other piece...");
	...
	static List<Transform> getAllControlPoints(Transform t){
		Debug.Log ("BG: Child_count of t: " + t.childCount); 
